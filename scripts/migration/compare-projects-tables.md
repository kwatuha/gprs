...
### Columns  principalinvestigator  principalinvestigatorstaffid programid subprogramid defaultphotoid voided 

 
map departmentid  to  ministry, sectionid to state_department, directorate to implementing_agency,
remove  principalinvestigator, principalinvestigatorstaffid,categoryid,
...
remove 
============================================
`projectname`  Maps to remote `name` |
| `projectdescription`  Maps to remote `description` |
| `directorate`  map to remote `implementing_agency` |
| `startdate` Should go into `timeline` JSONB |
| `enddate`  Should go into `timeline` JSONB |
| `costofproject` Should go into `budget` JSONB  on allocated_amount_kes |
 paidout Should go into `budget` JSONB  on allocated_amount_kes |
| `objective`  go into `notes` JSONB |
| `expectedoutput`  go into `notes` JSONB |
| `expectedoutcome`  go into `notes` JSONB |
| `status` Should go into `progress` JSONB |
| `statusreason`  Should go into `progress` JSONB |
| `createdat`  Maps to remote `created_at` |

| `departmentid`  map to remote `ministry` 
| `sectionid`  map to remote `state_department` |
| `finyearid`   go into `timeline` JSONB |
| `programid`  go into `notes` JSONB |
| `subprogramid`  go into `notes` JSONB |
...
| `approved_for_public` | boolean | YES | false | Should map to feedback_enabled into `public_engagement` JSONB |
...
## Real Example from Remote Database

From a sample project in the remote database:

```json
{
  "project_id": 10,
  "name": "Procurement and Deployment of 23,000 VDI Devices",
  "description": "A national initiative supporting the Jitume Digital Enablement Program...",
  "sector": "Digital Skills and Government Digitization",
  "implementing_agency": "Konza Technopolis Development Authority",
  "location": {
    "county": "National",
    "constituency": null,
  ...
  "budget": {
    "source": "Covered under Konza National Data Center and Smart City Project",
    "allocated_amount_kes": 0,
  ...
  "progress": {
    ...
    "latest_update_summary": "A total of 23,000 VDI devices were procured and deployed..."
  },
  "public_engagement": {
    "common_feedback": "",
    "feedback_enabled": true,
  ...
  "data_sources": [
    {
      "type": "government_program_report",
      "links": [],
      "last_synced": "",
      "retrieved_from": "State Department for ICT & Digital Economy – Jitume Digital Enablement Program",
      "verification_status": "Official"
    }
  ],
  "ministry": "Ministry of Information, Communications and the Digital Economy (MICDE)",
  "state_department": "State Department for ICT & Digital Economy"
}
```
